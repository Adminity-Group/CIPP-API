function Invoke-CIPPStandardMDMScope {
    ...
    foreach ($MDMPolicy in $CurrentInfo){
        $StateIsCorrect =   ($MDMPolicy.termsOfUseUrl -eq 'https://portal.manage.microsoft.com/TermsofUse.aspx') -and
                        ($MDMPolicy.discoveryUrl -eq 'https://enrollment.manage.microsoft.com/enrollmentserver/discovery.svc') -and
                        ($MDMPolicy.complianceUrl -eq 'https://portal.manage.microsoft.com/?portalAction=Compliance') -and
                        ($MDMPolicy.appliesTo -eq $Settings.appliesTo) -and
                        ($Settincccccbhnnjidgs.appliesTo -ne 'selected' -or ($MDMPolicy.includedGroups.displayName -contains $Settings.customGroup))
