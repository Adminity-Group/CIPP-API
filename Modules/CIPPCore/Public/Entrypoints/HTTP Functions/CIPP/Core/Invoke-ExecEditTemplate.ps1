...
function Invoke-ExecEditTemplate {
    ...
    try {
        ...
        elseif ($Type -eq 'ScriptTemplate'){
            ...
            Set-CIPPIntuneTemplate -RawJSON $RawJSON -GUID $GUID -PartitionKey $Type -DisplayName $Request.body.displayName -Description $Request.body.description -templateType $OriginalTemplate.Type -Headers $Request.Headers
        }
        else {
            $Table.Force = $true
