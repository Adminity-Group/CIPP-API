...
Function Invoke-ExecSetTenantShortName {
    ...
    Write-host "ExecSetTenantShortName_Request: $($Request | ConvertTo-Json -Depth 100)"
    Write-LogMessage -API $APINAME -tenant "Test" -headers $Request.Headers -message "$($Request| ConvertTo-Json -Depth 50)" -Sev 'Info'

    $body = [pscustomobject]@{'Results' = "Success. We've Tested Invoke-ExecSetTenantShortName." }
